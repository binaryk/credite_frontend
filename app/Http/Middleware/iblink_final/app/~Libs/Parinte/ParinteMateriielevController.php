...
class ParinteMateriielevController extends \Commons\DatatableController
{

	protected $id = 'parinte-materii-elev';
	...
	public function index($id = NULL)
	{
		...
		*/
		 ...
		 * sa vad mediile
		 **/
		$medii = $student->calculeazaMedii($materii, $this->semestres);
		...
		$config['caption'] = 'Catalog note | Clasa ' . $clasa->name . ' | <span title="' . $this->semester->start. ' - ' . $this->semester->end . '">Semestrul '  . $this->semester->name . '</span> | ' . $this->year->name;
		...
		return $this->show( $config + ['other-info' => [
			...
			'medii_clasa'            => $medii['medii_clasa'],
			'breadcrumb'             => \Helper::breadcrumb([
				['url' => \URL::to('/'), 'caption' => 'Acasă', 'active' => true],
				['url' => '#', 'caption' => 'Note', 'active' => false],
				['url' => route('index-parinte-absente-elev'), 'caption' => 'Absenţe', 'active' => true],
			])
		]]);
	}
	...
	public function getStudentGrades()
	{
		...
		return \Response::json(
			\App\Models\Libs\Noteabsente::getGrades(\Input::get('class_ids'), \Input::get('semester'))
		);
	}
	...
	{
		return \Response::json(
			\App\Models\Libs\Noteabsente::getAverages(
				\Input::get('class_ids'), 
